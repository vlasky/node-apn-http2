...
export class APNNotification extends APNNotificationBase {
  ...
  static readonly VALID_PRIORITIES = [1, 5, 10] as const;
  encoding = 'utf8';
  private _expiry: number | undefined = undefined;
  private _priority: 1 | 5 | 10 | undefined = undefined;
  private _pushType: typeof APNNotification.VALID_PUSH_TYPES[number] | undefined = undefined;
  private _topic: string | undefined = undefined;
  private _collapseId: string | undefined = undefined;
  private _id: string | undefined = undefined;
  private _pushTypeWarningShown = false;
  ...
  set priority(value: 1 | 5 | 10 | null | undefined) {
    ...
    } else if (!APNNotification.VALID_PRIORITIES.includes(value as 1 | 5 | 10)) {
      throw new Error("priority must be 1, 5, or 10");
    } else {
      this._priority = value as 1 | 5 | 10;
    }
  }
  ...
  set pushType(value: typeof APNNotification.VALID_PUSH_TYPES[number] | null | undefined) {
    ...
    } else if (!APNNotification.VALID_PUSH_TYPES.includes(value as typeof APNNotification.VALID_PUSH_TYPES[number])) {
      throw new Error(`pushType must be one of: ${APNNotification.VALID_PUSH_TYPES.join(', ')}`);
    } else {
      this._pushType = value as typeof APNNotification.VALID_PUSH_TYPES[number];
    }
  }
   ...
   */
  private hasAlertContent(): boolean {
    if (this.aps.alert == null) return false;
  ...
  private validatePriority(): void {
    ...
    if (this.priority === 10 && this.aps["content-available"] === 1 && !this.hasVisibleContent()) {
      throw new Error(
        "Invalid priority: priority 10 cannot be used with content-available notifications that have no alert, badge, or sound. " +
  ...
  headers(): Record<string, string | number> {
    ...
    if (this.pushType != null) {
      ...
      if (typeof this._mdm === 'string') {
        ...
        throw new Error(
          "Cannot determine push type automatically. " +
      ...
      console.warn(
        "rawPayload is set but pushType is not specified. " +
        "Set notification.pushType explicitly to 'alert', 'background', 'voip', 'mdm', etc. " +
    ...
    if (this.topic) {
      ...
      throw new Error(
        "Missing required 'topic' (your app's bundle ID). " +
  ...
  private apsPayload() {
    ...
    let alertValue: typeof this.aps.alert = this.aps.alert;
    if (alertValue && typeof alertValue === 'object') {
      const filteredAlert: Record<string, unknown> = {};
      ...
      alertValue = Object.keys(filteredAlert).length > 0 ? filteredAlert as typeof alertValue : undefined;
    }
    ...
    for (const key of Object.keys(aps)) {
      const value = aps[key as keyof typeof aps];
      if (value == null) {
        continue;
      }
  ...
  private validatePushType(): void {
    ...
    if (this.pushType === 'background') {
      if (this.aps["content-available"] !== 1) {
        throw new Error(
          "Invalid payload for background push: pushType is 'background' but content-available is not set to 1. " +
          "Background notifications require content-available: 1 in the aps payload."
        );
      }
      if (this.hasVisibleContent()) {
        throw new Error(
          "Invalid payload for background push: pushType is 'background' but notification has visible content (alert, badge, or sound). " +
    ...
    if (this.pushType === 'alert' && !this.hasVisibleContent()) {
      throw new Error(
        "Invalid payload for alert push: pushType is 'alert' but notification has no visible content. " +
    ...
    if (this.pushType === 'mdm' && typeof this._mdm !== 'string') {
      throw new Error(
        "Invalid payload for mdm push: pushType is 'mdm' but mdm payload is not set. " +
    ...
    if (typeof this._mdm === 'string' && this.pushType != null && this.pushType !== 'mdm') {
      throw new Error(
        `Invalid pushType '${this.pushType}' for mdm notification. ` +
  ...
  toJSON() {
    ...
    if (aps == null) {
      throw new Error(
        "Invalid notification: aps payload is empty. " +
